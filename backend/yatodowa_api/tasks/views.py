...
def add_list():
    tasks_list = TasksLists(name="list5")
    db.session.add(tasks_list)
    db.session.commit()
    return tasks_list.name, 201
...
def add_task():
    task = Tasks(
        text="lorem ipsum somthing somewhere haha", completed=False, list_name="list1"
    )
    db.session.add(task)
    db.session.commit()
    return task.text, 201
